...
from backend.agents import llm, input_agent, retrieval_agent, generation_severity_agent, hallucination_guard, output_agent
...
app = FastAPI(
    ...
    description="FastAPI clinical checker powered by an optimized clinical assessment pipeline.",
    version="1.1.0"
)
...
async def check_interactions(request: CheckRequest):
    """Executes the optimized clinical assessment pipeline with 2 LLM calls (Assessment + Audit)."""
    ...
    try:
        # ---- STEP 1: INPUT AGENT (Rule-Based Parser) ----
        t0 = time.time()
        ...
        agent1_res.logs.append(f"Step 1 Execution Time: {round(time.time() - t0, 3)}s")
        pipeline_steps.append(agent1_res)
        
        extracted = agent1_res.output_data
        drugs = extracted.get("drugs", [])
        drugs_recognized = extracted.get("drugs_recognized", [])
        drugs_unrecognized = extracted.get("drugs_unrecognized", [])
        ...
        # ---- STEP 2: RETRIEVAL AGENT (Database Lookup) ----
        t0 = time.time()
        ...
        agent2_res.logs.append(f"Step 2 Execution Time: {round(time.time() - t0, 3)}s")
        pipeline_steps.append(agent2_res)
        retrieval_data = agent2_res.output_data
        
        # ---- STEP 3: COMBINED CLINICAL ASSESSMENT AGENT (CALL 1) ----
        t0 = time.time()
        agent3_res = generation_severity_agent.run(
            drugs=drugs,
            conditions=conditions,
        ...
        agent3_res.logs.append(f"Step 3 (CALL 1) Execution Time: {round(time.time() - t0, 3)}s")
        pipeline_steps.append(agent3_res)
        generated_report = agent3_res.output_data
        
        # ---- STEP 4: HALLUCINATION GUARD (CALL 2) ----
        t0 = time.time()
        agent4_res = hallucination_guard.run(generated_report, retrieval_data)
        agent4_res.logs.append(f"Step 4 (CALL 2) Execution Time: {round(time.time() - t0, 3)}s")
        pipeline_steps.append(agent4_res)
        guard_data = agent4_res.output_data
        
        # ---- STEP 5: OUTPUT AGENT (Compilation & Disclaimers) ----
        t0 = time.time()
        agent5_res = output_agent.run(guard_data)
        agent5_res.logs.append(f"Step 5 Execution Time: {round(time.time() - t0, 3)}s")
        pipeline_steps.append(agent5_res)
        
        final_report_data = agent5_res.output_data
